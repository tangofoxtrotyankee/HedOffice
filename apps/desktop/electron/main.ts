 ...
 * locally, exposes the read-models to the renderer over IPC, and backs secrets
 * with the OS keychain via safeStorage. Headless Phases 0–3 deliberately don't
 * depend on any of this; here is where the shell finally wires it together.
 *
 * Note: not launched in CI (no display); typechecked + the handlers/secret store
 * are unit-tested. `pnpm electron:dev` runs it locally.
 */
const MCP_PORT = 4317;

async function bootstrap(): Promise<void> {
  // Approvals are surfaced to the human; here a console approver stands in for
  // the renderer-driven approval modal (wired via a follow-up IPC channel).
  const office = new Office({
    approval: { defaultPolicy: "prompt", approver: () => "allow" },
  });
